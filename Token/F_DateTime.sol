...
contract DateTimeEnabled {
	...
	function addDaystoTimeStamp(uint16 _daysToBeAdded) internal view returns(uint){
		return now + DAY_IN_SECONDS*_daysToBeAdded;
	}

	function addMinutestoTimeStamp(uint8 _minutesToBeAdded) internal view returns(uint){
		return now + MINUTE_IN_SECONDS*_minutesToBeAdded;
	}


	function printDatestamp(uint timestamp) internal pure returns (uint16,uint8,uint8,uint8,uint8,uint8) {
		DateTime memory dt;
		dt = parseTimestamp(timestamp);
		return (dt.year,dt.month,dt.day,dt.hour,dt.minute,dt.second);
	}
	
	function currentTimeStamp() internal view returns (uint) {
		return now;
	}
}
